...
grep -i -e "error" -e "exception" "$LOGFILE" | jq . 2>/dev/null | sort | uniq -c
echo

# Search for "warning" or "warn" to find warning messages
echo -e "${YELLOW}Warnings:${NC}"
grep -i -e "warn" "$LOGFILE" | jq . 2>/dev/null | sort | uniq -c
echo

# Finding entries related to specific services
echo -e "${BLUE}Backend Entries:${NC}"
grep -i "com.docker.backend.exe" "$LOGFILE" | jq . 2>/dev/null
echo

# Checking Start and Stop Times
echo -e "${GREEN}Start and Stop Times:${NC}"
grep -i -e "Docker started" -e "Docker stopped" "$LOGFILE" | jq . 2>/dev/null
echo
